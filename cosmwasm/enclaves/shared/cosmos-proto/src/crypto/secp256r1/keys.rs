...
impl PrivKey {
    ...
    pub fn get_ucpi(&self) -> &[u8] {
        &self.ucpi
    }
    pub fn clear_ucpi(&mut self) {
        self.ucpi.clear();
    }

    // Param is passed by value, moved
    pub fn set_ucpi(&mut self, v: ::std::vec::Vec<u8>) {
        self.ucpi = v;
    }

    // Mutable pointer to the field.
    // If field is not initialized, it is initialized with default value first.
    pub fn mut_ucpi(&mut self) -> &mut ::std::vec::Vec<u8> {
        &mut self.ucpi
    }

    // Take field
    pub fn take_ucpi(&mut self) -> ::std::vec::Vec<u8> {
        ::std::mem::replace(&mut self.ucpi, ::std::vec::Vec::new())
    }
}
...
impl ::protobuf::Message for PrivKey {
    ...
    fn merge_from(&mut self, is: &mut ::protobuf::CodedInputStream<'_>) -> ::protobuf::ProtobufResult<()> {
        while !is.eof()? {
            ...
            match field_number {
                1 => {
                    ::protobuf::rt::read_singular_proto3_bytes_into(wire_type, is, &mut self.ucpi)?;
                },
                _ => {
                    ::protobuf::rt::read_unknown_or_skip_group(field_number, wire_type, is, self.mut_unknown_fields())?;
                },
    ...
    fn compute_size(&self) -> u32 {
        let mut my_size = 0;
        if !self.ucpi.is_empty() {
            my_size += ::protobuf::rt::bytes_size(1, &self.ucpi);
        }
        my_size += ::protobuf::rt::unknown_fields_size(self.get_unknown_fields());
        self.cached_size.set(my_size);
        my_size
...
static file_descriptor_proto_data: &'static [u8] = b"\
    ...
    ucpi\x18\x01\x20\x01(\x0cR\x06ucpiB\x0b\xda\xde\x1f\x07ecdsaSKB@Z2gi\
