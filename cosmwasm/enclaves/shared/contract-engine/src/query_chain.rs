...
use crate::types::{IoNonce, ucpiMessage};
...
fn encrypt_query_request(
    ...
    }) = query_struct
    {
        ...
        let mut encrypted_msg = ucpiMessage {
...
fn decrypt_query_response(
    ...
    let as_ucpi_msg = ucpiMessage {
        nonce,
        user_public_key,
        msg: response,
    };

    let b64_decrypted = as_ucpi_msg.decrypt().map_err(|err| {
        debug!(
