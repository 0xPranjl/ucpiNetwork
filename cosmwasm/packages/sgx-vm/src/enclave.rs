...
fn init_enclave(enclave_file: &str) -> SgxResult<SgxEnclave> {
    ...
    let enclave_directory = env::var("ucpi_ENCLAVE_DIR").unwrap_or_else(|_| '.'.to_string());
    ...
    let enclave_file_path = enclave_file_path.ok_or_else(|| {
        warn!(
            "Cannot find the enclave file. Try pointing the ucpi_ENCLAVE_DIR environment variable to the directory that has {:?}",
            enclave_file
        );
